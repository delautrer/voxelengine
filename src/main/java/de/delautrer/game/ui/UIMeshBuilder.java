...
public class UIMeshBuilder {
    ...
    private void addClippedQuad(UITexture texture, float x0, float y0, float x1, float y1, float z, float r, float g, float b, float u0, float v0, float u1, float v1) {
    ...
    public void addCroppedAtlasQuad(float x, float y, float z, float w, float h, int gridX, int gridY, float cropRatio, float gridSize) {
        ...
        addClippedQuad(UITexture.UI, x, y, x + w, y + h, z, 1f, 1f, 1f,
                ...
                ((float) gridX / gridSize + epsilon) + (uCellWidth * cropRatio), (float) (gridY + 1) / gridSize - epsilon);
    }
    ...
    public void add9Slice(float x, float y, float z, float w, float h, int gridX, int gridY, float cornerRenderSize) {
        float epsilon = 0.0005f; float uvStep = 1.0f / UI_GRID;
        float u0 = (float) gridX * uvStep + epsilon; float v0 = (float) gridY * uvStep + epsilon;
        float u3 = u0 + uvStep - 2 * epsilon; float v3 = v0 + uvStep - 2 * epsilon;
        ...
        for (int row = 0; row < 3; row++) {
            for (int col = 0; col < 3; col++) {
                float px0 = posX[col]; float px1 = Math.max(posX[col+1], px0);
                float py0 = posY[row]; float py1 = Math.max(posY[row+1], py0);
    ...
    public float getTextWidth(String text, IFont font) {
        ...
        for (int i = 0; i < text.length(); i++) {
            char c = text.charAt(i);
            if (c >= 32 && c < 128) textWidth += charData.get(c - 32).xadvance();
        }
        return textWidth;
    }
