...
public class UIKeybindButton extends UIElement {
    ...
    public void handleInput(InputManager input, float mouseX, float mouseY) {
        ...
        if (isListening) {
            ...
            // Wir horchen nur auf Klicks, wenn die Maus EXAKT über dem kleinen Button
            // rechts ist!
    ...
    private String getKeyName(int keycode) {
        ...
        if (keycode == GLFW.GLFW_KEY_F12)
            return "F12";
        if (keycode == GLFW.GLFW_KEY_F13)
            return "F13";
        if (keycode == GLFW.GLFW_KEY_F14)
            return "F14";
        if (keycode == GLFW.GLFW_KEY_F15)
            return "F15";
        if (keycode == GLFW.GLFW_KEY_F16)
            return "F16";
        if (keycode == GLFW.GLFW_KEY_F17)
            return "F17";
        if (keycode == GLFW.GLFW_KEY_F18)
            return "F18";
        if (keycode == GLFW.GLFW_KEY_F19)
            return "F19";
        if (keycode == GLFW.GLFW_KEY_F20)
            return "F20";
        if (keycode == GLFW.GLFW_KEY_F21)
            return "F21";
        if (keycode == GLFW.GLFW_KEY_F22)
            return "F22";
        if (keycode == GLFW.GLFW_KEY_F23)
            return "F23";
        if (keycode == GLFW.GLFW_KEY_F24)
            return "F24";
        if (keycode == GLFW.GLFW_KEY_F25)
            return "F25";
