...
public class DecoratorPass implements IGenerationPass {
    ...
    public void process(Chunk chunk, long seed, int[][] heightMap) {
        ...
        if (treeChance >= 1.0f) {
            ...
            if (random.nextFloat() < (treeChance - (int) treeChance))
                numTrees++;
        } else {
        ...
        for (int x = 0; x < Chunk.SIZE; x++) {
            for (int z = 0; z < Chunk.SIZE; z++) {
                ...
                if (localBiome == null || localBiome == Biome.MOUNTAINS || localBiome.getFloraWeights() == null
                        || localBiome.getFloraWeights().isEmpty() || localBiome.floraDensity <= 0.0f)
                    continue;
                ...
                for (int y = Chunk.HEIGHT - 2; y >= 1; y--) {
                    ...
                    if (blockAtPos == uprightLog.getBlock().getId())
                        break;

                    if (blockAtPos == air.getBlock().getId() || blockAtPos == leaves.getBlock().getId()
                            || blockAtPos == water.getBlock().getId()) {
                        continue;
                    }
                    ...
                    if (soilDepth == 0 && y >= WATER_LEVEL) {
                            ...
                            if (patchNoise > localBiome.floraPatchThreshold) {
                                if (random.nextFloat() < localBiome.floraDensity) {
                                    ...
                                    if (localBiome == Biome.FLOWER_PLAINS && !flowerPlainsSelection.isEmpty()) {
                                        int patchTypeIndex = Math.abs((int) (patchNoise * 100))
                                                % flowerPlainsSelection.size();
                                    ...
                                    if (plantName != null) {
                                        BlockState plantToPlace = BlockRegistry
                                                .get(Constants.NAMESPACE + ":" + plantName).getDefaultState();
                                        if (plantToPlace != null) {
                                            chunk.setBlock(x, y + 1, z, plantToPlace.getBlock().getId(),
                                                    plantToPlace.getStateId());
                                        }
                                    }
                                }
                            }
                        ...
                        if (deepState != null) {
                            ...
                            if (blockAtPos == deepMaterialId) {
                                float bNoise = blobNoise.getFractalNoise3D(realX * 0.1f, y * 0.1f, realZ * 0.1f, 2,
                                        0.5f, 2.0f);
                                if (bNoise > 0.5f) {
                                    ...
                                    if (blobs != null) {
                                        for (Map.Entry<String, Float> entry : blobs.entrySet()) {
                                            if (random.nextFloat() < entry.getValue()) {
                                                BlockState blobBlock = BlockRegistry
                                                        .get(Constants.NAMESPACE + ":" + entry.getKey())
                                                        .getDefaultState();
                                                if (blobBlock != null) {
                                                    chunk.setBlock(x, y, z, blobBlock.getBlock().getId(),
                                                            blobBlock.getStateId());
                                                }
                                                break;
                                            }
                                        }
    ...
    private void generateTree(Chunk chunk, Random random, int x, int y, int z) {
            ...
            for (int lx = x - radius; lx <= x + radius; lx++) {
                for (int lz = z - radius; lz <= z + radius; lz++) {
                    ...
                    if (dx == radius && dz == radius) {
                        if (layer <= 1 && random.nextInt(2) == 0)
                            continue;
                        else if (layer == 3)
                            continue;
                    }

                    // BOUNDS CHECK: Verhindert Absturz, wenn der Baum in Nachbarchunks wachsen
                    // will!
        ...
        for (int ty = 0; ty < height; ty++) {
            // BOUNDS CHECK (Eigentlich durch die x,z Limits am Anfang schon sicher, aber
            // schadet nicht)
