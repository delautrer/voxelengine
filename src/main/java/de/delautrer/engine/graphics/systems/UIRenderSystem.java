...
public class UIRenderSystem implements IRenderSystem {
    ...
    public void render(VkCommandBuffer cmd, RenderPacket packet) {
        ...
        if (packet.uiCombinedMesh == null || ((VulkanMesh) packet.uiCombinedMesh).getIndexCount() == 0
                || packet.uiDrawCalls == null || packet.uiDrawCalls.isEmpty()) {
            return;
        }
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VK10.vkCmdBindVertexBuffers(cmd, 0, stack.longs(((VulkanMesh) packet.uiCombinedMesh).getVertexBuffer()),
                    stack.longs(0));
            VK10.vkCmdBindIndexBuffer(cmd, ((VulkanMesh) packet.uiCombinedMesh).getIndexBuffer(), 0,
