...
public class StarRenderSystem implements IRenderSystem {
    ...
    private void createPipeline(VulkanSwapchain swapchain, VulkanRenderPass renderPass) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            stages.get(0).sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO).stage(VK_SHADER_STAGE_VERTEX_BIT)
                    .module(vertModule).pName(stack.UTF8("main"));
            stages.get(1).sType(VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO).stage(VK_SHADER_STAGE_FRAGMENT_BIT)
                    .module(fragModule).pName(stack.UTF8("main"));
    ...
    public void render(VkCommandBuffer commandBuffer, RenderPacket packet) {
        if (packet.starMesh == null || ((VulkanMesh) packet.starMesh).getIndexCount() == 0
                || packet.starAlpha <= 0.0f) {
            return;
        }
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            VK10.vkCmdBindVertexBuffers(commandBuffer, 0, stack.longs(((VulkanMesh) packet.starMesh).getVertexBuffer()),
                    stack.longs(0));
            VK10.vkCmdBindIndexBuffer(commandBuffer, ((VulkanMesh) packet.starMesh).getIndexBuffer(), 0,
