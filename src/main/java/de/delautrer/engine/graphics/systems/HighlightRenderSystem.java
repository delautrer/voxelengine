...
public class HighlightRenderSystem implements IRenderSystem {
    ...
    public void render(VkCommandBuffer cmd, RenderPacket packet) {
        if (packet.selectedBlockPos == null || packet.highlightMesh == null
                || ((VulkanMesh) packet.highlightMesh).getIndexCount() == 0)
            return;

        VK10.vkCmdBindPipeline(cmd, VK10.VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline.getHandle());

        try (MemoryStack stack = MemoryStack.stackPush()) {
            // Die Model-Matrix muss jetzt auch relativ zur Kamera sein, wie beim
            // EntityRenderSystem
            ...
            // WICHTIG: Nutze packet.mvp (welche die Rotation beinhaltet), wie bei den
            // Entities
            ...
            VK10.vkCmdPushConstants(cmd, pipeline.getPipelineLayout(), VK10.VK_SHADER_STAGE_VERTEX_BIT, 0,
                    highlightMvpBuffer);

            VK10.vkCmdBindVertexBuffers(cmd, 0, stack.longs(((VulkanMesh) packet.highlightMesh).getVertexBuffer()),
                    stack.longs(0));
            VK10.vkCmdBindIndexBuffer(cmd, ((VulkanMesh) packet.highlightMesh).getIndexBuffer(), 0,
