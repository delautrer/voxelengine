...
import de.delautrer.engine.graphics.VulkanRenderPass;
import de.delautrer.engine.graphics.VulkanSwapchain;
...
public class SkyRenderSystem implements IRenderSystem {
    ...
    public void render(VkCommandBuffer commandBuffer, RenderPacket packet) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ByteBuffer pc = stack.malloc(112); // 112 Bytes für unsere Daten
            ...
            // 2. Sonne einfügen (Bytes 64 bis 75)
            pc.putFloat(64, packet.sunDirection.x);
            pc.putFloat(68, packet.sunDirection.y);
            pc.putFloat(72, packet.sunDirection.z);

            // 3. Zenith Color (Bytes 80 bis 91 - Offset 80 wegen GPU 16-Byte Ausrichtung)
            pc.putFloat(80, packet.skyR);
            pc.putFloat(84, packet.skyG);
            pc.putFloat(88, packet.skyB);

            // 4. Horizon Color (Bytes 96 bis 107)
            pc.putFloat(96, Math.min(packet.skyR + 0.05f, 1.0f));
            pc.putFloat(100, Math.min(packet.skyG + 0.05f, 1.0f));
            pc.putFloat(104, Math.min(packet.skyB + 0.05f, 1.0f));
